...
- [2026-04-04] MongoDB strict equality `{ isDeleted: false }` silently skipped older/unversioned database records missing the boolean entirely, creating a massive desync between admin panels and raw DB expectations. → Always query soft-deletes generically using `{ isDeleted: { $ne: true } }` so explicit false, null, and non-existent keys remain inclusive.
- [2026-04-04] A single malformed image URL in the DB completely crashed Next.js pages when fed to `<Image src={...}>` (e.g., throwing "Failed to construct 'URL'"). → Use native `<img>` tags combined with standard CSS when rendering arrays of unvalidated or potentially dirty URLs in dashboards to ensure UI resilience.
...
- [2026-04-09] Images added to the `public/` directory during a running dev session may return 404 (broken image icon) because Next.js's `<Image />` optimization feature caches the public directory state. → Restart the dev server (`npm run dev`) to force Next.js to re-index the public folder.
- [2026-04-09] Safari browsers can drop `overflow: hidden` clipping on elements with complex CSS transforms or button focus, revealing sharp image corners underneath rounded containers. → Use `[transform:translateZ(0)]` on both the container and the image wrapper to enforce hardware-accelerated clipping.
- [2026-04-09] Enhanced the Tour Planner modal with a "Liquid Glass" aesthetic using `backdrop-blur-32px` and `ring-white/50`. Replaced simple gradients with an image-backed modal header for maximum premium feel.
...
- Build-Tour place thumbnails now use dedicated generated assets in `/public/images/places/thumbs/` (104 JPGs, ~2.5MB total vs ~46MB originals). Planner/map UI should reference these thumbnail paths via `getPlaceThumbnailSrc()` and serve them directly with `unoptimized` `next/image` to avoid expensive on-demand resizing of the full place images.
